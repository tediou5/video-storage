...
pub type TaskFuture<'a, R> = Pin<Box<dyn Future<Output = R> + 'a>>;
...
pub struct StreamMap<'a, Index, R> {
    ...
    Index: Eq + Hash + Clone + Unpin,
{
    ...
    #[allow(dead_code)]
    pub fn push(&mut self, index: Index, fut: TaskFuture<'a, R>) {
    ...
    pub fn add_if_not_in_progress(&mut self, index: Index, fut: TaskFuture<'a, R>) -> bool {
        if self.in_progress.contains_key(&index) {
            false
        } else {
            self.in_progress
    ...
    fn poll_next_entry(&mut self, cx: &mut Context<'_>) -> Poll<Option<(Index, R)>> {
        if let Some((index, res)) = std::task::ready!(self.in_progress.poll_next_unpin(cx)) {
            ...
            self.process_queue(index.clone());
    ...
    fn process_queue(&mut self, index: Index) {
        if let Entry::Occupied(mut next_entry) = self.queue.entry(index.clone()) {
...
unsafe impl<'a, Index, R> Send for StreamMap<'a, Index, R>
where
    Index: Eq + Hash + Clone + Unpin,
    R: Send,
{
